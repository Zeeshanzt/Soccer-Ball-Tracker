...
class MinimalPublisherSubscriber(Node):
    ...
    def listener_callback(self, data):
        ...
        img2 = cv2.inRange(img_hsv, np.array(
            [19, 81, 0]), np.array([49, 255, 201]))
        rows = img2.shape[0]
        cols = img2.shape[1]
        img_interest = img2[int(rows - rows/10): int(rows), :]

        # Finding Centroid
        M4 = cv2.moments(img_interest)
        cx4 = 0
        cy4 = 0
        if M4['m00'] > 0:
            cx4 = int(M4['m10']/M4['m00'])
            cy4 = int(M4['m01']/M4['m00'])
        cv2.circle(img_interest, (cx4, cy4), 5, (0, 255, 0), 2)
        ...
        cv2.imshow('Interested Image', img_interest)
        cv2.waitKey(2)
